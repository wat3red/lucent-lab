...
void SetNextItemFullWidth() {
	const ImGuiStyle& style = ImGui::GetStyle();
	float width = ImGui::GetContentRegionAvail().x - style.WindowPadding.x;
...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
	...
	// --- Hover animation ---
	static std::unordered_map<ImGuiID, float> hover_t;
	float& t_hover = hover_t[id];
	if (hovered)
		t_hover += g.IO.DeltaTime * 6.0f;
	else
		t_hover -= g.IO.DeltaTime * 6.0f;
	t_hover = ImClamp(t_hover, 0.0f, 1.0f);

	// --- Press animation ---
	static std::unordered_map<ImGuiID, float> press_t;
	float& t_press = press_t[id];

	if (IsItemActive())
		t_press += g.IO.DeltaTime * 10.0f;
	else
		t_press -= g.IO.DeltaTime * 10.0f;

	t_press = ImClamp(t_press, 0.0f, 1.0f);

	auto easeInSine = [](float x) -> float {
		return 1.0f - cosf((x * IM_PI) / 2.0f);
		};

	float eased_hover = easeInSine(t_hover);
	float eased_press = easeInSine(t_press);

	// Interpolation: base -> hovered -> pressed
	ImVec4 base = style.Colors[ImGuiCol_Button];
	ImVec4 hover = style.Colors[ImGuiCol_ButtonHovered];

	// Press target: darker than hover
	ImVec4 pressedСolor = hover;
	pressedСolor.x *= 0.7f;
	pressedСolor.y *= 0.7f;
	pressedСolor.z *= 0.7f;

	// First interpolate hover over base
	ImVec4 temp;
	temp.x = ImLerp(base.x, hover.x, eased_hover);
	temp.y = ImLerp(base.y, hover.y, eased_hover);
	temp.z = ImLerp(base.z, hover.z, eased_hover);
	temp.w = ImLerp(base.w, hover.w, eased_hover) * g.Style.Alpha;

	// Then press over hover (or whatever is there)
	ImVec4 final_color;
	final_color.x = ImLerp(temp.x, pressedСolor.x, eased_press);
	final_color.y = ImLerp(temp.y, pressedСolor.y, eased_press);
	final_color.z = ImLerp(temp.z, pressedСolor.z, eased_press);
	final_color.w = ImLerp(temp.w, pressedСolor.w, eased_press) * g.Style.Alpha;

	ImU32 col = ColorConvertFloat4ToU32(final_color);
...
bool ImGui::Checkbox(const char* label, bool* v, const char* tooltipText)
{
	ImGuiWindow* window = GetCurrentWindow();
	if (window->SkipItems)
		return false;
	...
	// total width = full content region
	float full_width = window->WorkRect.Max.x - pos.x - style.WindowPadding.x;
	if (ImGui::GetCurrentWindow()->ScrollbarY)
		full_width -= style.ScrollbarSize;

	// place label on the left
	ImVec2 label_pos = pos;

	// place checkbox on the far right
	ImVec2 check_pos = ImVec2(pos.x + full_width - square_sz, pos.y);

	// bounding box covers the whole row
	ImRect total_bb(check_pos, ImVec2(pos.x + full_width, pos.y + square_sz));
	ItemSize(total_bb, style.FramePadding.y);
	...
	ImVec4 hover_col = style.Colors[ImGuiCol_FrameBgHovered];
	ImVec4 pressed_col = style.Colors[ImGuiCol_FrameBgActive];

	ImVec4 temp;
	temp.x = ImLerp(base.x, hover_col.x, eased_hover);
	temp.y = ImLerp(base.y, hover_col.y, eased_hover);
	temp.z = ImLerp(base.z, hover_col.z, eased_hover);
	temp.w = ImLerp(base.w, hover_col.w, eased_hover) * g.Style.Alpha;

	ImVec4 final_col;
	final_col.x = ImLerp(temp.x, pressed_col.x, eased_press);
	final_col.y = ImLerp(temp.y, pressed_col.y, eased_press);
	final_col.z = ImLerp(temp.z, pressed_col.z, eased_press);
	final_col.w = ImLerp(temp.w, pressed_col.w, eased_press) * g.Style.Alpha;

	ImU32 frame_col = ColorConvertFloat4ToU32(final_col);
	...
	static std::unordered_map<ImGuiID, float> check_t;
	float& t_check = check_t[id];
	if (*v)
		t_check += g.IO.DeltaTime * 10.0f;
	else
		t_check -= g.IO.DeltaTime * 10.0f;
	t_check = ImClamp(t_check, 0.0f, 1.0f);

	float eased_check = easeInSine(t_check);

	if (t_check > 0.0f)
	{
		ImVec4 col = style.Colors[ImGuiCol_CheckMark];
		col.w *= eased_check * g.Style.Alpha;
		ImU32 fill_col = ColorConvertFloat4ToU32(col);
	...
	// render label and tooltip
	if (label_size.x > 0.0f) {
		RenderText(label_pos, label);

		if (tooltipText) {
			ImGui::SetCursorScreenPos(ImVec2(
				label_pos.x + label_size.x + ImGui::GetStyle().ItemInnerSpacing.x,
				label_pos.y
			));
	...
	/*if (g.LogEnabled)
	{
		ImVec2 log_pos = label_pos;
		LogRenderedText(&log_pos, mixed_value ? "[~]" : *v ? "[x]" : "[ ]");
	}*/
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
	...
	if (!popup_open)
	{
		GetStateStorage()->SetFloat(ImHashStr("##ComboAnimTime", 0, popup_id), -1.0f);
	}
	...
	static std::unordered_map<ImGuiID, float> hover_t;
	float& t_hover = hover_t[id];
	t_hover += (hovered ? 1.0f : -1.0f) * g.IO.DeltaTime * 6.0f;
	t_hover = ImClamp(t_hover, 0.0f, 1.0f);

	// --- Press animation ---
	static std::unordered_map<ImGuiID, float> press_t;
	float& t_press = press_t[id];
	t_press += (held ? 1.0f : -1.0f) * g.IO.DeltaTime * 10.0f;
	t_press = ImClamp(t_press, 0.0f, 1.0f);

	auto easeInSine = [](float x) {
		return 1.0f - cosf((x * IM_PI) / 2.0f);
		};
	...
	ImVec4 base = style.Colors[ImGuiCol_Button];
	ImVec4 hover_col = style.Colors[ImGuiCol_ButtonHovered];
	ImVec4 press_col = ImVec4(hover_col.x * 0.7f, hover_col.y * 0.7f, hover_col.z * 0.7f, hover_col.w);

	ImVec4 temp_col = ImLerp(base, hover_col, eased_hover);
	ImVec4 final_col = ImLerp(temp_col, press_col, eased_press);
	final_col.w *= style.Alpha;

	const ImU32 col = ColorConvertFloat4ToU32(final_col);
	...
	if (preview_value != NULL)
		RenderTextClipped(bb.Min + style.FramePadding, bb.Max - style.FramePadding, preview_value, NULL, NULL, style.ButtonTextAlign, &bb);

	// Draw custom arrow ↓ (instead of triangle)
	ImVec2 arrow_center = ImVec2(bb.Max.x - style.FramePadding.x - arrow_size * 0.5f, bb.Min.y + style.FramePadding.y + arrow_size * 0.35f);
	float arrow_radius = 6.0f;
	ImU32 arrow_color = GetColorU32(ImGuiCol_Text);

	ImVec2 arrow_points[3] = {
		ImVec2(arrow_center.x - arrow_radius, arrow_center.y - arrow_radius * 0.5f),
		ImVec2(arrow_center.x, arrow_center.y + arrow_radius * 0.5f),
		ImVec2(arrow_center.x + arrow_radius, arrow_center.y - arrow_radius * 0.5f)
	};

	window->DrawList->AddPolyline(arrow_points, 3, arrow_color, ImDrawFlags_None, 1.8f);

	if (!popup_open)
		return false;

	g.NextWindowData.Flags = backup_next_window_data_flags;
	//ResetComboPopupAnimIfClosed(popup_id);
...
bool ImGui::BeginComboPopup(ImGuiID popup_id, const ImRect& bb, ImGuiComboFlags flags)
{
	...
	// 1. Параметры анимации
	const float animation_duration = 0.18f; // Длительность анимации в секундах

	// 2. Отслеживание времени начала анимации
	// Используем StateStorage для хранения времени, когда анимация началась для этого конкретного попапа.
	ImGuiID anim_time_id = ImHashStr("##ComboAnimTime", 0, popup_id);
	float anim_start_time = GetStateStorage()->GetFloat(anim_time_id, -1.0f);

	if (anim_start_time == -1.0f)
	{
		// Если время не установлено, значит, попап только что открылся.
		// Записываем текущее время.
		anim_start_time = (float)g.Time;
		GetStateStorage()->SetFloat(anim_time_id, anim_start_time);
	}

	// 3. Вычисление прогресса анимации
	const float t_elapsed = (float)g.Time - anim_start_time;
	float t = ImSaturate(t_elapsed / animation_duration); // Прогресс от 0.0 до 1.0

	// 4. Применение Easing-функции для плавности
	// Используем Cubic Ease-Out: анимация начинается быстро и плавно замедляется к концу.
	t = 1.0f - powf(1.0f - t, 3.0f);

	// --- КОНЕЦ БЛОКА АНИМАЦИИ ---

	// Set popup size (стандартный код ImGui)
	float w = bb.GetWidth();
	...
	// MODIFIED: Измененная логика позиционирования
	if (ImGuiWindow* popup_window = FindWindowByName(name))
		if (popup_window->WasActive)
		{
			ImVec2 size_expected = CalcWindowNextAutoFitSize(popup_window);

			// Анимируем размер по высоте
			// MODIFIED: Вместо фиксированного размера используем интерполированный
			size_expected.y *= t;

			popup_window->AutoPosLastDirection = (flags & ImGuiComboFlags_PopupAlignLeft) ? ImGuiDir_Left : ImGuiDir_Down;
			ImRect r_outer = GetPopupAllowedExtentRect(popup_window);

			// Вычисляем финальную позицию, как в оригинале
			ImVec2 pos_final = FindBestWindowPosForPopupEx(bb.GetBL(), size_expected, &popup_window->AutoPosLastDirection, r_outer, bb, ImGuiPopupPositionPolicy_ComboBox);

			// MODIFIED: Создаем начальную позицию
			// Начальная позиция - верх попапа совмещен с верхом комбобокса
			ImVec2 pos_start = pos_final;
			pos_start.y = bb.Min.y - size_expected.y; // Начальное положение "внутри" комбобокса

			// MODIFIED: Интерполируем позицию от начальной к конечной
			// Позицию Y будем сдвигать не полностью, а на высоту самого комбо, чтобы создать эффект выезда
			pos_final.y = ImLerp(bb.Min.y, pos_final.y, t);

			SetNextWindowPos(pos_final);
			SetNextWindowSize(size_expected); // Устанавливаем анимированный размер
		}
	...
	// MODIFIED: Анимируем прозрачность для более плавного появления
	PushStyleVar(ImGuiStyleVar_WindowPadding, ImVec2(g.Style.FramePadding.x, g.Style.WindowPadding.y));
	PushStyleVar(ImGuiStyleVar_Alpha, g.Style.Alpha * t); // Прозрачность от 0 до 1

	bool ret = Begin(name, NULL, window_flags);

	PopStyleVar(2); // Снимаем Alpha и WindowPadding
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
	...
	// Detect manual input before computing sizes
	const bool temp_input_allowed = (flags & ImGuiSliderFlags_NoInput) == 0;
	const bool temp_input_active = temp_input_allowed && TempInputIsActive(id);

	// Height logic
	const float base_slider_height = g.FontSize * 0.6f;
	const float slider_height = temp_input_active ? g.FontSize * 1.1f : base_slider_height; // increase if input active
	const float total_height = slider_height + g.FontSize + style.ItemInnerSpacing.y;

	const ImVec2 pos = window->DC.CursorPos;
	const ImRect frame_bb(pos + ImVec2(0, g.FontSize + style.ItemInnerSpacing.y),
		pos + ImVec2(w, g.FontSize + style.ItemInnerSpacing.y + slider_height));
	const ImRect total_bb(pos, pos + ImVec2(w, total_height));

	ItemSize(ImVec2(total_bb.GetWidth(), total_height), style.FramePadding.y);
	if (!ItemAdd(total_bb, id, &frame_bb, temp_input_allowed ? ImGuiItemFlags_Inputable : 0))
		return false;
	...
	// Manual input logic
	bool temp_input_is_active = temp_input_active;
	if (!temp_input_is_active)
	{
		const bool input_requested_by_tabbing = temp_input_allowed && (g.LastItemData.StatusFlags & ImGuiItemStatusFlags_FocusedByTabbing) != 0;
		const bool clicked = (hovered && g.IO.MouseClicked[0]);
	...
	temp.y = ImLerp(base.y, hover_col.y, eased_hover);
	temp.z = ImLerp(base.z, hover_col.z, eased_hover);
	temp.w = ImLerp(base.w, hover_col.w, eased_hover) * style.Alpha;

	ImVec4 final_col;
	final_col.x = ImLerp(temp.x, pressed_col.x, eased_press);
	final_col.y = ImLerp(temp.y, pressed_col.y, eased_press);
	final_col.z = ImLerp(temp.z, pressed_col.z, eased_press);
	final_col.w = ImLerp(temp.w, pressed_col.w, eased_press) * style.Alpha;
	...
	// Filled part
	float filled_x = grab_bb.Max.x;
	ImU32 filled_col = GetColorU32(ImGuiCol_SliderGrab);
	window->DrawList->AddRectFilled(frame_bb.Min, ImVec2(filled_x, frame_bb.Max.y), filled_col, style.FrameRounding);

	// Texts (value + label on first line)
	char value_buf[64];
	const char* value_buf_end = value_buf + DataTypeFormatString(value_buf, IM_ARRAYSIZE(value_buf), data_type, p_data, format);
	...
	if (label_size.x > 0.0f)
	{
		ImVec2 label_pos = ImVec2(frame_bb.Min.x, pos.y);
		window->DrawList->AddText(label_pos, GetColorU32(ImGuiCol_Text), label, FindRenderedTextEnd(label));
	}

	ImVec2 value_pos = ImVec2(frame_bb.Max.x - value_size.x, pos.y);
	window->DrawList->AddText(value_pos, GetColorU32(ImGuiCol_Text), value_buf);
...
bool ImGui::SliderScalarN(const char* label, ImGuiDataType data_type, void* v, int components, const void* v_min, const void* v_max, const char* format, ImGuiSliderFlags flags)
{
	...
	PushMultiItemsWidths(components, width);
	size_t type_size = GDataTypeInfo[data_type].Size;
	for (int i = 0; i < components; i++)
	{
		PushID(i);
	...
	{
		SameLine(0, g.Style.ItemInnerSpacing.x);
		SetCursorPosX(GetCursorPosX() - width);
		TextEx(label, label_end);
	}
