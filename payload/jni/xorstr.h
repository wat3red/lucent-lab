...
namespace jm {
    ...
    namespace detail {
        ...
        load_xored_str8(uint64_t key, std::size_t idx, const CharT* str) noexcept
        {
    ...
    template<class CharT, std::size_t Size, uint64_t... Keys, std::size_t... Indices>
    ...
    public:
        ...
        XORSTR_FORCEINLINE void crypt() noexcept
        {
            ...
            alignas(alignment)
                uint64_t arr[]{ ::jm::detail::load_from_reg(Keys)... };
            uint64_t*    keys =
                (uint64_t*)::jm::detail::load_from_reg((uint64_t)arr);
#else
