...
struct ImVec2
{
	...
	//ImVec2 operator* (float k) const { return ImVec2(x * k, y * k); }
	//ImVec2 operator/ (float k) const { return ImVec2(x / k, y / k); }
...
struct ImVec4
{
	...
	ImVec4 operator* (ImVec4 k) const { return ImVec4(x * k.x, y * k.y, z * k.z, w * k.w); }
...
namespace ImGui
{
	...
	IMGUI_API bool          Checkbox(const char* label, bool* v, const char* tooltipText = nullptr);
	...
	void					Tooltip(const char* fmt, ...) IM_FMTLIST(1);
